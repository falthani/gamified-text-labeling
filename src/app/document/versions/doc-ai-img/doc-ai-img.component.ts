...
@Component({
  selector: 'app-doc-ai-img',
  templateUrl: './doc-ai-img.component.html',
  styleUrls: ['./doc-ai-img.component.scss'],
})
export class DocAiImgComponent implements OnInit {
  ...
  async addPic() {
    ...
    await Promise.all(
      this.posTagItems.map(async (item) => {
        //had to skip pronouns
        if (item.pos === 'NOUN' || item.pos === 'PROPN' || item.pos === 'PRON') {
          validPosCounter++;
          const imagePath = validPosCounter;
          const filePath =
            this.filename +
            '-imgs/' +
            this.filename +
    ...
    const validPosItemCount = this.getValidPosCount();
    console.log('Number of items that are NOUN, PRON, or PROPN:', validPosItemCount);
