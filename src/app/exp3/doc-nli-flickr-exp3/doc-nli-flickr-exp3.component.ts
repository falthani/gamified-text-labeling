...
@Component({
  selector: 'app-doc-nli-flickr-exp3',
  templateUrl: './doc-nli-flickr-exp3.component.html',
  styleUrls: ['./doc-nli-flickr-exp3.component.scss']
})
export class DocNliFlickrExp3Component {
  ...
  async addPic() {
    try {
      const imagePromises = this.data.map(async (item) => {
        const filePath =
        'nil-final/flickr/' + item.id + '.webp'; // Assuming the image file path is derived from the id
