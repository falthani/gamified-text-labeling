...
@Component({
  selector: 'app-doc-nli-img',
  templateUrl: './doc-nli-img.component.html',
  styleUrls: ['./doc-nli-img.component.scss'],
})
export class DocNliImgComponent implements OnInit {
  ...
  async addPic() {
    try {
      const imagePromises = this.data.map(async item => {
        const filePath = 'nli-snli-flickr-high-imgs/' + item.id + '.webp'; // Assuming the image file path is derived from the id
