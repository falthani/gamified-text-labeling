...
@Component({
  selector: 'app-doc-nli-gr-exp1',
  templateUrl: './doc-nli-gr-exp1.component.html',
  styleUrls: ['./doc-nli-gr-exp1.component.scss'],
})
export class DocNliGrExp1Component implements OnInit {
  ...
  async addPic() {
    try {
      const imagePromises = this.data.map(async (item) => {
        const filePath =
          'mlni-imgs/mlni-imgs-relevant/' + 'gr-' + item.id + '.webp'; // Assuming the image file path is derived from the id
