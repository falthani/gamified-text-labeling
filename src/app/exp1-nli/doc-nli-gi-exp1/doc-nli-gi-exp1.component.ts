...
@Component({
  selector: 'app-doc-nli-gi-exp1',
  templateUrl: './doc-nli-gi-exp1.component.html',
  styleUrls: ['./doc-nli-gi-exp1.component.scss'],
})
export class DocNliGiExp1Component implements OnInit {
  ...
  async addPic() {
    try {
      const imagePromises = this.data.map(async (item) => {
        const filePath =
          'mlni-imgs/mlni-imgs-irrelevant/' + 'gi-' + item.id + '.webp'; // Assuming the image file path is derived from the id
  ...
  openBottomSheet(): void {
    const bottomSheetRef = this.bottomSheet.open(DocNliDialogComponent);
